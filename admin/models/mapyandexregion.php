...
class MapYandexModelmapyandexregion extends JModelAdmin
{
/**
* Foobar ID
*
...
/**
	...
	public function getForm($data = array(), $loadData = true) 
	{
		...
		$def = $this->getFoobar();
		if($def) {
			$def = json_decode($def->map_region_style);	
		} else {
			$def[0] = 1;
		}
		$form = $this->loadForm('com_mapyandex.mapyandexregion', 'mapyandexroute',
		                        array('control' => 'jform', 'load_data' => $loadData));
		$form->setFieldAttribute( 'map_region_opacity', 'default', $def[0] );
		if (empty($form)) 
		{
			return false;
		}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_mapyandex.edit.mapyandexregion.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
...
*/
	function getAllRoute()
	{
		...
		if (!$this->_allroute)
		{
			...
			." WHERE ".$db->nameQuote('map_type')." = 'map'";
			$db->setQuery($query);
...
*/
	function getFoobar()
	{
		...
		if (!$this->_foobar)
		{
			$db = $this->getDBO();
			$query = "SELECT * FROM ".$db->nameQuote('#__map_yandex') 
			." WHERE ".$db->nameQuote('id')." = ".$this->_id." AND ".$db->nameQuote('map_type')." = 'map'";
			$db->setQuery($query);
	...
	{	
		...
		// Store the web link table to the database
			...
			." set ".$db->nameQuote('region_map_yandex')." = '".$this->textnl($el)
			."', ".$db->nameQuote('map_region_style')." = '".$style."' WHERE ".$db->nameQuote('id')." = ".$data['id'];
