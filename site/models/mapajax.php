...
class MapYandexModelmapajax extends JModel
{
	...
	*/
	...
	{	
		...
		// Store the web link table to the database
			...
			if(!$data['editmarket']) {
			$query = "INSERT INTO ".$db->nameQuote('#__map_yandex_metki') 
			...
			('".$data['name_marker']."','".$data['misil']."','".$data['misilonclick']."','".$data['city_map_yandex']."', '".$data['street_map_yandex']."', 0, '$gmt', 4, 1, 0, 1, '', '".$data['yandexcoord']."','$lng', '".$id."', '".$data['deficon']."', '".$user."', '".$userimg."')";
			} else {
			$query = "UPDATE ".$db->nameQuote('#__map_yandex_metki') 
			." set ".$db->nameQuote('name_marker')." = '".$data['name_marker']
			."', ".$db->nameQuote('misil')." = '".$data['misil']
			."', ".$db->nameQuote('misilonclick')." = '".$data['misilonclick']
			...
			."', ".$db->nameQuote('id_map')." = '".$data['id_map']
			."', ".$db->nameQuote('deficon')." = '".$data['deficon']
			."', ".$db->nameQuote('whoadd')." = '".$user
